...
export const sendEnquiryResponse = createAsyncThunk(
  "enquiry/send-enquiry-response",
  async ({ enquiryId, comments, user }, thunkAPI) => {
    try {
      const response = await axios.post(
        `http://localhost:8088/enq-allo/save`,
        null,
        {
          params: {
            enquiryId,
...
export const enquirySlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(sendEnquiryResponse.pending, (state) => {
        state.isLoading = true;
      })
      .addCase(sendEnquiryResponse.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isError = false;
        state.isSuccess = true;
        state.responses.push(action.payload);
      })
      .addCase(sendEnquiryResponse.rejected, (state, action) => {
        state.isLoading = false;
        state.isError = true;
        state.isSuccess = false;
        state.message = action.error.message;
